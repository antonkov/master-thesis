...
\begin{frame}{Report \\ VI Конгресс молодых ученых}
\begin{block}{Content}
\begin{enumerate}
...
\begin{itemize}
  \item Check hypothesis of correlation between number of short cycles and iterative decoding effectiveness
  \item Develop algorithm for Tanner's graph spectrum (number of fixed length cycles) calculation
\end{itemize}
\end{frame}
...
\begin{frame}{Preliminaries \\ Linear code}
  Linear $(n,k)$ code \\
  $G$ -- generator matrix \\
  $H$ -- paritiy check matrix \\
  $G \cdot H^T = 0$ -- paritiy check equation \\
...
\begin{itemize}
  ...
  \item Line between if that bit is involved in that parity equation
\end{itemize}
...
\begin{frame}{Conclusion}
  \begin{itemize}
    \item Experiments confirmed correlation and showed that codes with lower number of short cycles has
      lower frame error rate.
    \item Computationally effective algorithm for Tanner's graph spectrum calculation was developed and
